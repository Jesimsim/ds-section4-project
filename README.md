...
## [ 의의 ] 
**Fine Tune을 통해 매우 향상된 모델 성능**
- 기존모델에 비해 Rouge Score가 매우 개선됨(Pretrianed 0.31 > Fine tune 0.8)

**다양한 방법을 활용하여 결과 도출 및 비교**
- TextRank(추출요약 알고리즘)과 KoBART(추상요약 DL모델), 두 방법 상의 차이 및 장단점을 확인
- NLG 모델에 적합한 점수체계(Rouge Score)를 사용하여 성능 비교
- 다양한 토크나이저를 활용하여 결과 비교

## [ 한계점 ]
**데이터셋 자체의 한계**
  - 학습셋과 테스트셋의 구조/형태가 비슷해 과적합 우려가 있어, 다양한 데이터셋을 확보하여 학습 및 성능평가를 통해 일반화 성능을 높여야 할 필요성이 있음

**Metric 보완** 
- 한국어 Text Generatoin 평가에 적절한 RDASS, ROUGE-SU 등을 구현하여 평가
...
## [ 보완점 ]
- 질의응답 모델까지 구현하여 서비스의 완결성을 높이고 싶음.
